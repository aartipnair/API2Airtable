...
class OuraringActivity:
    ...
    def __init__(self) -> None:
        ...
        self.ouraring_activity_base_name = config.ouraring_activity_base_name
        self.ouraring_activity_base_cols = config.ouraring_activity_base_cols
        self.ouraring_baseurl = config.ouraring_baseurl
        self.ouraring_activity_endpoint = config.ouraring_activity_endpoint
    ...
    def get_ouraring_api_data(self,apikey):    
        ...
        try:
            ...
            url = self.ouraring_baseurl+self.ouraring_activity_endpoint
    ...
    def check_for_update(self,date):
       
        res = None
        
        try:
            url = f"{self.base_url}/{self.base_id}/{self.ouraring_activity_base_name}"
            query_params = "?filterByFormula=({summary_date}='"+date+"')"              
    ...
    def prepare_airtable_data(self,app_id,rows):
        ...
        for row in rows:
            ...
            body ={'fields':{}}
            for key in self.ouraring_activity_base_cols:
    ...
    def update_ouraring_Data(self,users):
        ...
        try:            
            ...
            for user in users:                    
                ...
                if not ouraring_data['activity']:
                    logging.error(f'No data from activity api for user : {username}')
                    continue

                req_body= self.prepare_airtable_data(app_id,ouraring_data['activity'])  
                
                logging.info(f"{username} : processing {len(ouraring_data['activity'])}/{len(req_body['records'])} ")                 
    ...
    def update_data_in_airtable(self,id,req_body): 
        ...
        try:
            url = f'{self.base_url}/{self.base_id}/{self.ouraring_activity_base_name}/{id}'
            headers = { 'Authorization': f'Bearer {self.airtable_api_key}',
                            'Content-Type': 'application/json'                            }
